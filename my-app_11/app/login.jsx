...
const LoginScreen = () => {
    ...
    const handleLogin = async () => {
        if (name.trim() == "Jivan" && password.trim() == "123") {
            await SecureStore.setItemAsync("token", "Patel Jivan");
    ...
    const handleBiometric = async () => {
        ...
        if(!token || !biometric) {
            Alert.alert("Invalid Credentials", "Login with username and password");
            return;
        }
        ...
        if(!hasHardware) {
            Alert.alert("Failed", "Biometric does not exist");
            return;
        }
        ...
        if(!isEnrolled) {
            Alert.alert("Fingerprint and face lock is not enrolled");
            return;
        }

        const result = await LocalAuthentication.authenticateAsync({
            promptMessage: "Login with face lock/fingerprint",
    ...
    return (
        <View style={styles.container}>
            ...
            >
                <Text style={styles.buttonText}>Login with face lock / fingerprint</Text>
            </Pressable>
        </View>
    )
}
